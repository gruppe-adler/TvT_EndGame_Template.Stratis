...
//spawn function ===============================================================
_grad_endgame_common_fnc_spawnULGroup = {
...
sleep 2;
{
  ...
  /*if (count _houseList > 0) then {
    ...
    for [{_i = 0}, {_i < _houseAmount}, {_i=_i+1}] do {
      _position = selectRandom ([selectRandom _houselist] call grad_endgame_common_fnc_getBuildingPositions);
      _units = [selectRandom _availableUnits];
      [_side, _position, _units, false] call _grad_endgame_common_fnc_spawnULGroup;
  ...
  while {_spawnsLeft > 0} do {
    ...
    [_side, _position, _units, _patrol] call _grad_endgame_common_fnc_spawnULGroup;
  };
